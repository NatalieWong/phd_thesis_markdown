...
![TensorFlow Object Detection models - comparisons](https://lh3.googleusercontent.com/yBxte6onUHFr8lIcYNmJoMaGDWie9mKQFdIrGJr8UT0ThV17wAuZRFLwz7-9XitXWvw8OXjDhhq0=s550 "TensorFlow models - comparisons")
...
The following chart shows the relationship between the confidence of hand detection results (in %) and the models' total number of training steps. Higher confidence means that the model has a higher accuracy in the detection of a hand.
...
![rpi3-v1-v2-mydataset-confidence](https://lh3.googleusercontent.com/gDET1rpTGtdHTSZ5tH6yhLG6c7_a8aPPi3WHo5r8AnhYMrqmL3rNTBsR-6rEFz2GOYe3y2Mxz9dR=s680 "rpi3-v1-v2-mydataset-confidence") \

To conclude, a good hand detection model for mobile application, which is able to locate a hand accurately and with minimal delay for real-time hand detection, can only be generated by using a quality dataset, and re-training a pre-trained object detection model with higher accuracy and low latency like quantized SSD MobileNet V2.
...
- hands can be clearly seen,
- dimension of all the images should be consistent with the images captured by the camera for inference, and
- no resizing is needed as all the resizing methods involve interpolation on the original information of the images.
...
Even though my dataset has images collected from the videos of people playing the piano, typing words on the keyboard and folding Origami models having more gestures which suit my purpose, the Egohands dataset still outperforms my own dataset.

The Egohands dataset has more than 4,401 images (around 83% of total images) for model training and all the images are already in a dimension of 1280 x 720. Annotations for all the hands that appear in each image are done.

In contrast, for my own dataset, I only have 1,537 images (80% of the total images collected from 7 videos) for model training and some images with an original dimension of 640 x 360 have to be resized to 1280 x 720. Only one hand can be detected and annotated for each image.
